...
//=================================================================================================
    ...
    //float pos0[] = {-1.127, -1.632, 0.172, -0.007, -0.147, 0, 0.0983};
    //float pos1[] = {-1.0544, -0.716, -0.399, -0.672, -0.124, 0, 0.103};
    ...
    //float ready[] = {-0.58028, -1.5993, 1.6527, 1.1033, 0.09986, -0.04688, 0.3526};

    jp_type jp_wam = wam.getJointPositions();
    ...
    while(true)
    {
		...
		if (user_input == 'V')
		{
            ...
            float dig_in_a[] = {-0.491787, -1.41935, 1.71565, 1.47543, 0.0227725, -0.330043, -0.396903}; //
            float dig_in_b[] = {-0.799642, -1.31445, 1.73819, 1.96903, -0.100974, -0.903863, -0.399267};

            float joint5_up[] = {-0.491787, -1.41935, 1.71565, 1.47543, 0.0227725, -0.330043 + 0.2, -0.396903}; //
            float joint5_down[] = {-0.491787, -1.41935, 1.71565, 1.47543, 0.0227725, -0.330043 - 0.2, -0.396903}; //

            float joint6_up[] = {-0.491787, -1.41935, 1.71565, 1.47543, 0.0227725, -0.330043, 0+0.2}; //

            float arm_a[] = {-0.491787, -1.41935, 1.71565-0.25, 1.47543, 0.0227725, -0.330043, -0.396903}; //
            float arm_b[] = {-0.491787, -1.41935, 1.71565-0.25, 1.47543+0.25, 0.0227725, -0.330043, -0.396903}; //
            float arm_c[] = {-0.491787, -1.41935, 1.71565, 1.47543+0.25, 0.0227725, -0.330043, -0.396903}; //
            ...
            for (int i=0; i<2; i++)
            {
                ...
                std::cout << "[Enter] to move to dig_in_b\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = dig_in_b[i];
                wam.moveTo(jp_wam);
            ...
            for (int i=0; i<2; i++)
            {
                std::cout << "[Enter] to move to arm_a\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = arm_a[i];
                wam.moveTo(jp_wam);

                std::cout << "[Enter] to move to arm_b\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = arm_b[i];
                wam.moveTo(jp_wam);

                std::cout << "[Enter] to move to arm_c\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = arm_c[i];
                wam.moveTo(jp_wam);
            ...
            for (int i=0; i<2; i++)
            {
            std::cout << "[Enter] to move to joint5_up\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = joint5_up[i];
                wam.moveTo(jp_wam);

            std::cout << "[Enter] to move to joint5_down\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = joint5_down[i];
                wam.moveTo(jp_wam);
            }
            ...
            for (int i=0; i<2; i++)
            {
            std::cout << "[Enter] to move to joint6_up\n";
                if(!demo_mode) while(getchar() != '\n');
                for (size_t i = 0; i < 7; i++)
                    jp_wam[i] = joint6_up[i];
                wam.moveTo(jp_wam);
            ...
            std::cout << "[Enter] to move to standby position\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = standby2[i];
            wam.moveTo(jp_wam);
        ...
        {
            while(getchar() != '\n');
            float explore1u[] = {-0.337695, -1.2921, 1.69317, 1.59653, 0.0779642, -0.198943, -0.354162};
            float explore1d[] = {-0.536382, -1.32156, 1.74238, 1.31837, -0.00142328, -0.166682, -0.3481};

            float explore2u[] = {-0.234297, -1.54134, 1.72145, 1.89711, 0.231916, -0.399547, -0.396801};
            float explore2d[] = {-0.512423, -1.54894, 1.73422, 1.44322, 0.172533, -0.243065, -0.397109};

            float explore3u[] = {-0.626704, -1.22697, 1.60117, 2.11919, -0.232786, -0.890183, -0.390328};
            float explore3d[] = {-0.725756, -1.28412, 1.65239, 1.85151, -0.104453, -0.730855, -0.3893};

            float explore4u[] = {-0.65249, -1.45614, 1.76086, 2.29049, 0.00403263, -1.07972, -0.385396};
            float explore4d[] = {-0.76206, -1.47707, 1.78818, 1.93486, 0.00648384, -0.889867, -0.384574};

            std::cout << "\n\n\n\n\n~~~~~~EXPLORE MODE~~~~~~\n\n";

            std::cout << "[Enter] to move to explore 1 up\n";
            while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore1u[i];
            wam.moveTo(jp_wam);
            std::cout << "[Enter] to move to explore 1 down\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore1d[i];
            wam.moveTo(jp_wam);
            if(demo_mode) usleep(1500000);
            std::cout << "[Enter] to move to explore 1 up\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore1u[i];
            ...
            std::cout << "[Enter] to move to explore 2 up\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore2u[i];
            wam.moveTo(jp_wam);
            std::cout << "[Enter] to move to explore 2 down\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore2d[i];
            wam.moveTo(jp_wam);
            if(demo_mode) usleep(1500000);
            std::cout << "[Enter] to move to explore 2 up\n";
            if(!demo_mode) while(getchar() != '\n');
            ...
            std::cout << "[Enter] to move to explore 3 up\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore3u[i];
            wam.moveTo(jp_wam);
            std::cout << "[Enter] to move to explore 3 down\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore3d[i];
            wam.moveTo(jp_wam);
            if(demo_mode) usleep(1500000);
            std::cout << "[Enter] to move to explore 3 up\n";
            if(!demo_mode) while(getchar() != '\n');
            ...
            std::cout << "[Enter] to move to explore 4 up\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore4u[i];
            wam.moveTo(jp_wam);
            std::cout << "[Enter] to move to explore 4 down\n";
            if(!demo_mode) while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = explore4d[i];
            wam.moveTo(jp_wam);
            if(demo_mode) usleep(1500000);
            std::cout << "[Enter] to move to explore 4 up\n";
            if(!demo_mode) while(getchar() != '\n');
...
*/
        ...
        if (user_input == 'C')
        {
            ...
            float retrieve[] = {-0.533789, -1.3625, 1.72843, 1.69241, 0.00885597, -0.453078, -0.401732};
            float retrieve_away[] = {0.118226, -1.42565, 1.55711, 2.28427, -0.127542, -0.465808, 0.0251725};
            
            std::cout << "\n\n\n\n\n~~~~~~RETRIEVE MODE~~~~~~\n\n";

            std::cout << "[Enter] to move to retrieve position\n";
            while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = retrieve[i];
            wam.moveTo(jp_wam);

            std::cout << "[Enter] to move to retrieve away\n";
            while(getchar() != '\n');
            for (size_t i = 0; i < 7; i++)
                jp_wam[i] = retrieve_away[i];
            wam.moveTo(jp_wam);
        ...
        if (user_input == 'T')
        {
            ...
            while (true)
            {
                ...
                std::cout << "x : Exit Tele Op\n";
                std::cin >> tele;

                jp_wam = wam.getJointPositions();

                if (tele == 'q') jp_wam[0] += incr;
                if (tele == 'a') jp_wam[0] -= incr;
                if (tele == 'w') jp_wam[1] += incr;
                if (tele == 's') jp_wam[1] -= incr;
                if (tele == 'e') jp_wam[2] += incr;
                if (tele == 'd') jp_wam[2] -= incr;
                if (tele == 'r') jp_wam[3] += incr;
                if (tele == 'f') jp_wam[3] -= incr;
                if (tele == 't') jp_wam[4] += incr;
                if (tele == 'g') jp_wam[4] -= incr;
                if (tele == 'y') jp_wam[5] += incr;
                if (tele == 'h') jp_wam[5] -= incr;
                if (tele == 'u') jp_wam[6] += incr;
                if (tele == 'j') jp_wam[6] -= incr;
